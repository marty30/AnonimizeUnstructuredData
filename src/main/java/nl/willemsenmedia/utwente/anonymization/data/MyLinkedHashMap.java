...
public class MyLinkedHashMap<K, V> extends LinkedHashMap<K, V> {
	...
	public synchronized V put(K key, V value) {
		...
		if (keyList.size() != this.size()) {
			...
			System.err.println("De lengte van de keylist en de hashmap is niet meer gelijk. Dat is raar! Het verschil (wel in keylist maar niet in keyset): " + StringUtils.join(keys_in_keyList, ", ") + ". (wel in keyset maar niet in keyList): " + StringUtils.join(keys_in_keyset, ", ") + ".");
		} else if (!keySet().equals(new HashSet<>(keyList))) {
			System.err.println("De keylist en de keyset zijn niet meer gelijk. Dat is raar!");
		}
		return old_value;
	}
